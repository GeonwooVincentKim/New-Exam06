...
int main(int argc, char **argv) {
  ...
  for (;;) {
    ...
    memset(sendbuff, 0, sendbuff_size);
    ...
    ready = select(clients_no + 1 + 1, &readfds, NULL, NULL, NULL);
    if (ready == -1) {
      fatal();
    }
    ...
    if (FD_ISSET(sockfd, &readfds)) {
      ...
      if (fd == -1) {
        ...
        broadcast(sendbuff, clients, clients_no, fd);
        appear_clients++;
        continue;
      }
    }
    ...
    for (int i = 0; i < appear_clients; i++) {
      ...
      if (fd > 0 && FD_ISSET(fd, &readfds)) {
        nbytes = recv(fd, recvbuff, recvbuff_size, 0);
        if (nbytes < 1) {
          close(fd);
          clients[i] = 0;
          sprintf(sendbuff, "server: client %d just left\n", i);
          broadcast(sendbuff, clients, clients_no, 0);
